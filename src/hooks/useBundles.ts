  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("bundles")
        .select("*, products(name, name_ar, image_url, sale_price, cost_price, shipping_price, category_id, sku, is_published, categories(name)), bundle_items(*, product_variations(name, sku))")
...
export function useBundle(id: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("bundles")
        .select("*, products(name, name_ar, image_url, sale_price, cost_price, shipping_price, category_id, sku, description, description_ar, short_description, short_description_ar, is_published, product_type, gallery_images, categories(name)), bundle_items(*, product_variations(name, sku, price))")
        .eq("id", id!)
        .single();
      if (error) throw error;
      return data;
